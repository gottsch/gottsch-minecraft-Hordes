...
import com.someguyssoftware.hordes.horde.Horde;
...
public class TickHandler {
	...
	// TODO move all server code to world - just ensure that we only process one of
	// the world's ticks ie. Overworld.
	/**
	 * Using the WorldTickEvent as opposed to the Server tick event because the
	 * WorldTickEvent has a natural access to the world object.
	 * 
	 * @param event
	 */
	@SubscribeEvent
	public void onWorldTick(TickEvent.WorldTickEvent event) {
		// TODO need manage for multiple dimensions... each dimension get's it own
		// HordeMind ?
		// only process on the server
		// ensure we only process overworld ticks
		if (event.world.provider.getDimension() != 0 || event.world.isRemote) {
			return;
		}

		Integer dimensionId = event.world.provider.getDimension();

		boolean isMaxPopulationMet = true;

		HordeMind mind = HordeMindManager.getHordeMind(dimensionId);
		...
		for (Entry<String, Horde> entry : mind.getHordes().entrySet()) {
			...
			for (Hordestone stone : horde.getHordestones()) {
				...
				// TODO this needs to move - or add a flag that is set if met and reset for
				// every horde
				if (mind.getTicks() > HordesProperties.ticksPerHordeMindUpdate) {
					Hordes.logger.debug("Mind ticks=" + mind.getTicks());

					Hordes.logger.debug("\tChecking update for Horde Hordestone " + horde.getId() + " @ " + stone.getX()
					...
					// check for building
					if (horde.getCurrentBuildings() < horde.getMaxBuildings()
							&& stone.getBuildTicks() > HordesProperties.ticksPerBuild) {
					...
					if (stone.getHarvestTicks() > HordesProperties.ticksPerHarvest) {
						...
						// TODO calculate percentages of each resources to allocate to each category ex.
						// building materials, food

						// TODO calcualte percentage of building materials go to each type - wood,
						// stone, glass, dirt, etc.
					...
					// TODO check the current number of hordestones against max and the time
					// elapsed.

					// check for job reassignment - would have to have a list of every entity
					// connected to this stone.... or every entity in the entire horde.
